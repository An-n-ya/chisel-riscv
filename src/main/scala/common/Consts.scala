...
object Consts {
  ...
  val MEN_LEN = 2
  val MEN_X   = 0.U(MEN_LEN.W) // memory write disabled
  val MEN_S   = 1.U(MEN_LEN.W) // scalar
  val MEN_V   = 2.U(MEN_LEN.W) // vector

  val REN_LEN = 2
  val REN_X   = 0.U(REN_LEN.W) // register write back disabled
  val REN_S   = 1.U(REN_LEN.W) // register write back scalar
  val REN_V   = 2.U(REN_LEN.W) // register write back vector
