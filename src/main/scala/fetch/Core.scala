...
class Core extends Module {
  ...
  val csr_wdata = MuxCase(
    ...
    Seq(
      ...
      (csr_cmd === CSR_E) -> 11.U(WORD_LEN.W) // machine mode ECALL
    )
  )
  when(csr_cmd > 0.U) {
    csr_regfile(csr_addr) := csr_wdata
    printf(
      p"write csr addr: 0x${Hexadecimal(csr_addr)}, data: 0x${Hexadecimal(csr_wdata)}\n"
    )
  }
...
//  printf(p"dmem.wdata: 0x${Hexadecimal(io.dmem.wdata)}\n")
  ...
  // printf(p"a3: ${Hexadecimal(regfile(13))}\n")
  // printf(p"a4: ${Hexadecimal(regfile(14))}\n")
